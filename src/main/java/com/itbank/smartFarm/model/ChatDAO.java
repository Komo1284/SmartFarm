...
public interface ChatDAO {
    ...
    @Select("SELECT m.*, u.nick, m.timestamp, m.sender_id, m.receiver_id FROM message m JOIN member u ON m.sender_id = u.id " +
    ...
    @Select("SELECT DISTINCT u.* " +
            "FROM message m " +
            "JOIN member u ON (m.sender_id = u.id OR m.receiver_id = u.id)" +
            "WHERE (m.sender_id = #{receiverId} OR m.receiver_id = #{receiverId})" +
            "AND u.id != #{receiverId}")
